...
from src.text import process_txt,  process_doc, process_pdf, preprocess_text, split_into_chunks
...
def process_files(files: List[dict], config):
    ...
    for file in files:
        try:
            ...
            filepath = os.path.join(config.FILES_FOLDER, file.get('filename'), file.get('ext'))
